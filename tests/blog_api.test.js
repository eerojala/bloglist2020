...
describe('When there is an user saved to the database', () => {
  const userData = { username: 'user1', name: 'User 1', password: 'password1' }
  let user 
  
  beforeAll(async () => {
    await User.deleteMany({})

    user = await helper.saveUser(userData) // returns a User object with id included in field '_id' (note that user does not have the plaintext password as a field)
  })
  ...
  describe('When the user is logged in', () => {
    ...
    describe('POST /api/blogs', () => {
      test('a valid blog can be added', async () => {
        ...
        const blogsWithoutIdsAndUsers = helper.removeUserFields(blogsWithoutIds)
